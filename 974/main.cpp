...
/*  Editorial - explaining prefixSum
prefixSum = A * k + R0
    A is quotient
    and R0 is the remainder when divided by k
...
Here are two visual examples showing the calculations:
                    ...
                    prefixSum[j]       = 2 + 3 + 4 + 6 + (-1) = 14
                    prefixSum[j] mod k = 14 % 3               = 2
                     vj
nums    [2][3][4][6][-1][2][3]
            ^i
    prefixSum[i]       = 2 + 3 = 5
    prefixSum[i] mod k = 5 % 3 = 2

    since prefixSum[j]%k == prefixSum[i]%k, we can see the subarray from i+1 to j, is divisible by k

                        
                    prefixSum[j]       = 2 + 3 + 4 + 6 + (-1) + 2 = 16
                    prefixSum[j] mod k = 14 % 3                   = 1
                         vj
nums    [2][3][4][6][-1][2][3]
            ^i
    prefixSum[i]       = 2 + 3 = 5
    ...
    since prefixSum[j]%k != prefixSum[i]%k, we can see the subarray from i+1 to j, is not divisible by k
